    ...
    def process_request(self, peer_id, request_id, uri):
        ...
        if uri in self.isac_node.isac_values:
            ...
            if isac_value.metadata:
                logger.info('(%s) Responding to metadata survey for %s: %s',
                            self.isac_node.name, uri, self.isac_node.name_uuid())
                self.reply(peer_id, request_id, (isac_value.metadata, self.isac_node.name_uuid()))
            else:
                logger.info('(%s) I know %s but I don\'t have any metadata for it',
                            self.isac_node.name, uri)
        else:
