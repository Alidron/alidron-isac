    ...
    def process_request(self, peer_id, request_id, uris, is_re=False):
        ...
        if is_re:
            ...
            try:
                ...
                return
            uris = list(filter(match_filter.search, value_uris))
        ...
        if all_metadata:
            logger.debug(
                '(%s) Can respond to metadata survey for %d values',
                self.isac_node.name, len(all_metadata.keys())
            )
            self.reply(peer_id, request_id, all_metadata)
        else:
            logger.debug(
                '(%s) I don\'t have anything to respond to metadata survey', self.isac_node.name)
